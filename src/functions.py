...
def remove_personal_info(category):
    orders_df = pa.read_csv(f"{category}.csv",sep=",",names=["datetime","location","customer_name","product","payment_method","amount_paid","credit_card_provider"])
    orders_df = orders_df.drop(columns="customer_name")
    orders_df['order_id'] = np.arange(orders_df.shape[0])

    credit_card_number_list = orders_df["credit_card_provider"].tolist()
    orders_df = orders_df.drop(columns="credit_card_provider")
    new_list = []
    ...
    orders_df['credit_card_provider'] = pa.Series(new_list)
...
def write_into_dataframe(the_tuples):
    ...
    value_dictionary = {"product": product_list, "order_id" : order_id_list}
